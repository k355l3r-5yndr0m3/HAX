...
capture' top idx tf ranked@(RankedTensorType shape t e) value
  | top == idx = return (ranked, value)
  | top >  idx = 
    case tf of 
      Id          -> error "Unexpectedly short transformation stack"
      V dim other -> do 
        -- NOTE: Stupid code, the order of transforms to be applied is opposite
        ...
        let _map    = BroadcastMap $ take (length shape) [1..]
            ranked' = RankedTensorType (dim:shape) t e
        value' <- SHLO._BroadcastInDimOp _map value (toAnyType ranked')
        capture' (top - 1) idx other ranked' value'
...
instance TensorOp Tracer where
  ...
  dot  :: forall s t. (T s t, Num t) => Tracer s t -> Tracer s t -> Tracer '[] t
  dot lhs rhs = Tracer tag $ \ t0 (transformTruncate tag -> tf) -> do 
    let _type = toAnyType $ applyTransform tf $ tensorType (Proxy :: Proxy (Tracer '[] t))
        _attr = applyTransform tf $ DotDimensionNumbersAttr { getBatchingDims = [], getContractingDims = take rank [(i, i) | i <- [0..]] }
    (t1, _lhs) <- takevar lhs t0 tf tag
    (t2, _rhs) <- takevar rhs t1 tf tag
    (t2, ) <$> SHLO._DotGeneralOp _attr Nothing _lhs _rhs _type
    where tag   = max (getTag lhs) (getTag rhs)
...
instance T s t => VMap (Tracer s t) where
  ...
  vmap' terminatorLabel delayed = Tracer terminatorLabel $ \ tbl (transformTruncate terminatorLabel -> tf) -> 
