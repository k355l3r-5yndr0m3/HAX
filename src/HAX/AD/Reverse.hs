...
instance (ShapeOp r, KnownShape s) => Reversable (Reverse r s Bool) where
...
instance (ShapeOp r, KnownShape s) => Reversable (Reverse r s Int64) where
  type ReversedType (Reverse r s Int64) = r s Int64
  constructReverse i t = (i + 1, Reverse (t, nograd))
  gradReify _ i (Gradient gs) = (i + 1, splat 0, g')
    where (_, Gradient -> g') = partition ((i ==) . fst) gs

instance (ShapeOp r, KnownShape s) => Reversable (Reverse r s Word8) where
